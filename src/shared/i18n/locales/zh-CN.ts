...
export const zhCN = {
  assistant: {
    ...
    proposal: { accept: { create: '新建文档', delete: '删除文档', edit: '接受并保存' }, original: '原文', preview: '审阅对比', proposed: '建议稿', reject: '拒绝', status: { applying: '正在保存已接受的修改……', conflict: '磁盘文件已变化，提案未应用。', created: '已创建接受的新文档。', deleted: '已删除接受的文档。', failed: '无法应用此提案。', missing: '文件已被移动或删除，提案未应用。', rejected: '已拒绝提案。', saved: '已保存接受的修改。', stale: '提案生成后项目或草稿已有变化，请让 Agent 重新读取。' }, title: { create: '新建文档“{{title}}”的提案', delete: '删除文档“{{title}}”的提案', edit: '对“{{title}}”的修改提案' } },
    ...
    tools: { hideDetails: '收起详情', input: '调用参数', names: { get_current_document: '读取当前文档', get_document: '读取文档', get_novel_structure: '读取小说结构', propose_document_edit: '提交文档修改提案', propose_document_file_operation: '提交新建或删除文档提案' }, output: '调用结果', showDetails: '查看详情', status: { cancelled: '已取消', completed: '已完成', failed: '失败', running: '调用中' } },
