...
export const buildAgentSystemPrompt = (
  ...
  const capabilityInstructions = context.availableTools.length === 0
    ...
    : [
        ...
        'When a reviewed manuscript edit or creation made for a writing request is accepted, read accepted_reconciliation context so Main supplies the exact persisted document and compact UUID-free story refs, then explicitly check Personae, Chronicle, Threads, and open questions in turn, even when one needs no change. Avoid duplicates. Prefer reconcile_accepted_document for one depicted Chronicle event and its advances to existing Threads; it binds the accepted source, revisions, ordering, IDs, and links atomically. Use ordinary Maintain only for clear shapes the focused tool cannot represent, and record author judgments as questions. After all applicable work, call complete_story_reconciliation with the faithful outcome; an accepted Scribe-backed manuscript workflow cannot finish before that checkpoint succeeds. Do not ask the user to approve routine synchronization or promote unaccepted prose to canon.',
        ...
        'If reconciliation finds a possible alias, uncertain time, unclear relationship, contradiction, or any other fact requiring author judgment, do not change canonical story records. Record one deduplicated structured story question with exact evidence when available; for the accepted document, use its document:accepted ref so Main binds the hidden ID and revision. Then raise unresolved questions concisely in the response. Group related questions and ask only what materially affects the story. When the user explicitly answers an open question, resolve that question and only then apply any resulting unambiguous low-risk story maintenance.',
  ...
  const delegationInstructions = context.availableTools.includes('delegate_writing')
      ? [
        'A writing delegation is the single bounded Scribe child task available for this user request, not permission to persist or expand the work. Supply one precise assignment and never call or retry delegate_writing a second time. Review the returned Markdown without narrating the review. Use revise_writing_artifact only for directly verified typos or formatting defects, never for continuity, gender, tone, or phrasing judgments; copy find strings verbatim, and if an exact revision is rejected, do not retry it. Then use the ordinary reviewed proposal workflow.',
