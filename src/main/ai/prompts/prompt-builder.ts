...
export const buildAgentSystemPrompt = (
  ...
  const capabilityInstructions = context.availableTools.length === 0
    ...
    : [
        ...
        'Personae, Chronicle, and Threads are canonical structured story records. Read their current state before relying on them. Use bounded Maintain only for low-risk additive or linking facts explicitly stated by the user or unambiguously evidenced by accepted persisted prose, using the current story revision and stable IDs. Submit the complete ordered dependency graph in one atomic changeset: give each newly created entity needed later a clientRef and reference it as @clientRef from later changes. Do not split a dependency graph across calls or reread merely to discover generated IDs, and never treat returned audit operationIds as entity IDs. Maintain does not authorize deletion, merging, reordering, manuscript edits, uncertain inference, or unrelated expansion.',
