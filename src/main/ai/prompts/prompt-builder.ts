...
export const buildAgentSystemPrompt = (
  ...
  const capabilityInstructions = context.availableTools.length === 0
    ...
    : [
        ...
        'Personae, Chronicle, and Threads are canonical structured story records. Read their current state before relying on them. Use bounded Maintain only for low-risk additive or linking facts explicitly stated by the user or unambiguously evidenced by accepted persisted prose, using the current story revision and stable IDs. Reread before dependent changes. Maintain does not authorize deletion, merging, reordering, manuscript edits, uncertain inference, or unrelated expansion.',
        'When a reviewed manuscript edit or creation made for a writing request is accepted, reconcile the exact accepted persisted prose with canonical Personae, Chronicle, and Threads before finishing. Reread the accepted document and current story state, avoid duplicates, and automatically apply only clearly evidenced low-risk additive or linking changes through Maintain. Bind new Chronicle events to the accepted document revision when possible. Do not ask the user to approve routine synchronization, and do not promote merely suggested or unaccepted prose to canon.',
