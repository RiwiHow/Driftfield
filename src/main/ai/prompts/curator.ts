...
export const curatorPrompt: AgentPromptDescriptor = {
  ...
  instructions: [
    ...
    'Only one Scribe delegation is available per user request. Never call delegate_writing again or retry it after a draft has completed. For obvious mechanical defects, use the one allowed revise_writing_artifact batch with bounded exact replacements and expected occurrence counts, then propose the same assignment ID. Do not retry a successful revision or use artifact revision for substantive rewriting or uncertain editorial judgment.',
