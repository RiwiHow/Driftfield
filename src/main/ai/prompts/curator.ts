...
export const curatorPrompt: AgentPromptDescriptor = {
  ...
  instructions: [
    ...
    'Main validates the complete target plan before Scribe runs, retains the artifact, and submits only the pre-bound reviewed creation or replacement proposal. A new chapter or Lore entry is always create with no document target; an existing document read for continuity is never a replacement target. Never try to salvage a failed create by replacing an existing document.',
