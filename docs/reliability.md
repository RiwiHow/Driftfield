...
- `PROJECT.json`, project-relative Markdown, `STORY.json`, `ICONS.txt`, and the
...
- Accepted Scribe-backed Manuscript reconciliation exposes `ACCEPTED.md` and
  `STORY.json` in Bash. Main requires explicit inspection of both, resolves the
...
- Lore-category icon changes require a category path from the latest snapshot
  and an exact icon listed in `ICONS.txt`. Acceptance updates
